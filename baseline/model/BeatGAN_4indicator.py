        ...
        with open(os.path.join(self.outf, self.model, self.dataset, "val_info.txt"), "w") as f:
            for epoch in range(self.niter):
                ...
                if (pre + rec) > best_result:
                    ...
                    #save_ts_heatmap_1D(self.input.detach().cpu().numpy(), self.fake.detach().cpu().numpy(), 'BeatGAN_{}'.format(self.cur_epoch))
                ...
                if (Pre_test + Rec_test ) <= early_stop_results:
                    ...
                    early_stop_epoch = 0
                    early_stop_results = (Pre_test + Rec_test)
                    torch.save(self.G.state_dict(), '/home/changhuihui/learn_project/COCA/baseline/Model_CheckPoint/{}_{}_{}_{}.pkl'.format(self.model, self.dataset, self.normal_idx,self.seed))
    ...
    def predict(self,dataloader_,scale=True):
        with torch.no_grad():
            ...
            if self.opt.istest == True:
                plot_hist(self.an_scores.cpu().numpy(), self.gt_labels.cpu().numpy(), self.opt,'beatgan_hist', display=False)
                plot_tsne_sns(self.latent_i.cpu().numpy(), self.gt_labels.cpu().numpy(), self.opt,'beatgan_latent', display=False)
