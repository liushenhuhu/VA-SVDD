...
import numpy as np
from metric import  evaluate as evaluation
...
def training(epochs, model, train_loader, val_loader, test_loader, opt_func=torch.optim.Adam):
    history = []
    ...
    for epoch in range(epochs):
        ...
        #     best_result_epoch = epoch
            ...
            ap_test, auc_test, Pre_test, Rec_test, F1_test = testing(model, test_loader, .5, .5)
        ...
        #     if epoch == 1:
        #         early_stop_results = (Pre_test + Rec_test + F1_test)
        #
        # if (Pre_test + Rec_test + F1_test) <= early_stop_results:
        #     early_stop_epoch = early_stop_epoch + 1
        # else:
        #     early_stop_epoch = 0
        #     early_stop_results = (Pre_test + Rec_test + F1_test)
        ...
        #         epoch, pre, rec, f1, best_pre, best_rec, best_f1,best_result_epoch, Pre_test, Rec_test , F1_test, early_stop_epoch))
    ...
    #return ap_test, auc_test,Pre_test,Rec_test,F1_test, best_result_epoch
...
def testing(model, test_loader, alpha=.5, beta=.5):
    ...
    auc_prc, roc_auc, Pre, Recall, f1= evaluation(labels,results)
...
def validating(model, val_loader, alpha=.5, beta=.5):
    ...
    ap, auc, Pre, Recall, F1 = evaluation(labels,results)

    return ap, auc, Pre, Recall, F1
