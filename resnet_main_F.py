...
optimizer = optim.Adam(
            [
                {"params": resnet18_ft.parameters()}],
            lr=LR, betas=(0.5, 0.999))
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=lr_decay_step, gamma=0.9)  # 设置学习率下降策略
...
for epoch in range(start_epoch + 1, MAX_EPOCH):
    ...
    for data in dataloader["train"]:
        # set input
        inputs = torch.cat((data[3],data[4],data[5]),0)
    ...
    with torch.no_grad():
        ...
        if acc>maxACC:
            ...
            result["acc"] = acc
            torch.save(result, './resnet_F.pth')
